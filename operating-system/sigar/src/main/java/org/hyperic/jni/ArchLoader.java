...
public class ArchLoader {
    ...
    private boolean isJarURL(URL url) {
        ...
        if (ix != -1) {
            ...
            if (ix != -1) {
                ...
                this.version = jarName.substring(jarName.indexOf('-') + 1, ix);
    ...
    private String findJarPath(String libName, boolean isRequired) throws ArchLoaderException {
        ...
        if (url == null) {
            if (isRequired) {
                throw new ArchLoaderException("Unable to find " + getJarName());
            } else {
                return null;
            }
        }
        ...
        while ((file != null) && !file.getName().startsWith(jarName)) {
    ...
    protected boolean findNativeLibrary(String dir, String name) {
        ...
        return containsNativeLibrary(path, getLibraryPrefix() + getName() + getLibraryExtension());
    }
    ...
    protected void loadLibrary(String path) throws ArchNotSupportedException, ArchLoaderException {
        try {
            ...
            String msg = "Failed to load " + libName + ": " + reason;
