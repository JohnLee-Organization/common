...
public class ChineseCalendar {
     ...
     * @return 月份
     */
    public static int nextChineseMonth(int year, int month) {
     ...
     * @return 期限
     */
    public static int sectionalTerm(int year, int month) {
     ...
     * @return 期限
     */
    public static int principleTerm(int year, int month) {
        if (year < 1901 || year > 2100)
            return 0;
        int index = 0;
        int ry = year - baseYear + 1;
    ...
    public String[] getYearTable() {
        ...
        table[0] = StringUtil.center(String.format("公历年历：%d", gregorianYear), 84);
        table[1] = StringUtil.center(
                String.format(
                        "农历年历：%d (%s%s - 年)",
                        chineseYear + 1,
        ...
        table[ln] = StringUtil.center("", 84);
        ln++;
        table[ln] = StringUtil.left("##/## - 公历日期/农历日期，(*)#月 - (闰)农历月第一天", 84);
        ln++;
        return table;
    }
