...
import net.lizhaoweb.common.util.http.common.HttpHeader;
import net.lizhaoweb.common.util.http.common.Utils;
import net.lizhaoweb.common.util.http.exception.HttpProcessException;
import net.lizhaoweb.common.util.http.httpclient.HttpClientUtil;
...
public class OCR {
    ...
    private static final String apiKey = "OCR.key";
    private static final String boundary = "----------------------------OcrKing_Client_Aven_s_Lab";
    private static final String end = "\r\n--" + boundary + "--\r\n";
    private static final Header[] headers = HttpHeader.custom().accept("text/html, image/gif, image/jpeg, *; q=.2, */*; q=.2").userAgent("Mozilla/5.0 (Windows NT 5.1; zh-CN; rv:1.9.1.3) Gecko/20100101 Firefox/8.0").contentType("multipart/form-data; boundary=" + boundary).referer("http://lab.ocrking.com/?javaclient0.1)").build();
    private static final Map<String, Object> map = getParaMap();
    private static HttpClient client = null; // =HCB.custom().proxy("127.0.0.1",
    // 8888).build();
    ...
    public static String ocrCode(String imgFilePath, int limitCodeLen) {
        ...
        String html;
        try {
            html = HttpClientUtil.post(HttpConfig.custom().client(client).url(apiUrl).requestHeaders(headers).requestParameters(m));
    ...
    public static String ocrCode4Net(String imgUrl, int limitCodeLen) {
        ...
        try {
            String html = HttpClientUtil.post(HttpConfig.custom().client(client).url(apiUrl).requestHeaders(headers).requestParameters(map));
            // System.out.println(html);
            String[] results = StringUtil.regex("<Result>([^<]*)</Result>\\s*<Status>([^<]*)</Status>", html);
            if (results.length > 0) {
                // System.out.println(results[0]);
    ...
    public static String ocrCode4Net(HttpConfig config, String savePath, int limitCodeLen) {
        ...
        String html;
        try {
            html = HttpClientUtil.post(config.client(client).url(apiUrl).requestHeaders(headers).requestParameters(m));
            // System.out.println(html);
            String[] results = StringUtil.regex("<Result>([^<]*)</Result>\\s*<Status>([^<]*)</Status>", html);
            if (results.length > 0) {
                // System.out.println(results[0]);
