...
import net.lizhaoweb.common.aliyun.oss.search.config.BoxConfigOfflineV1;
 ...
 */
public class SearchFromOSSForSavorServiceOfflineV1 implements ISearchFromOSSForSavorService {

    private IOSSClient ossClient;

    public SearchFromOSSForSavorServiceOfflineV1(IOSSClient ossClient) {
        super();
    ...
    public void boxLogExistFromOSSSimpleAll() {
        ...
        Pattern getBoxMacPattern = Pattern.compile(BoxConfigOfflineV1.BOX_REGEX_ALL);
        List<String> boxMacList = Arrays.asList(BoxConfigOfflineV1.BOX_MAC_LOG_ARRAY);
        String startMarker = null;
        boolean isHasMore = false;
        do {
            OSSObjectSummaryList ossObjectSummaryList = ossClient.listObjects(GlobalConfig.BUCKET_NAME, "log/box-standalone/", startMarker, BoxConfigOfflineV1.KEYS_SIZE);
    ...
    public void boxLogExistFromOSSSimple() {
        ...
        Pattern getBoxMacPattern = Pattern.compile(BoxConfigOfflineV1.BOX_REGEX_SIMPLE);
        List<String> boxMacList = Arrays.asList(BoxConfigOfflineV1.BOX_MAC_ARRAY);
        Calendar startCalendar = DateUtil.stringToCalendar(BoxConfigOfflineV1.DATE_START, "yyyyMMdd");
        Calendar endCalendar = DateUtil.stringToCalendar(BoxConfigOfflineV1.DATE_END, "yyyyMMdd");
...
//            System.out.println(DateUtil.calendarToString(indexCalendar,"yyyy-MM-dd HH;mm:ss"));
            String prefix = String.format("log/box-standalone/010/%s/", DateUtil.calendarToString(indexCalendar, "yyyyMMdd"));
            String startMarker = null;
            boolean isHasMore = false;
            do {
                OSSObjectSummaryList ossObjectSummaryList = ossClient.listObjects(GlobalConfig.BUCKET_NAME, prefix, startMarker, BoxConfigOfflineV1.KEYS_SIZE);
                List<OSSObjectSummary> ossObjectSummaryList_ = ossObjectSummaryList.getList();
...
//                    }
    ...
    public void boxLogExistFromOSSDetail() {
        try {
            byte[] contentBytes = BoxConfigOfflineV1.BOX_INFO.getBytes(Constant.Charset.UTF8);
            ...
            Pattern getBoxMacPattern = Pattern.compile(BoxConfigOfflineV1.BOX_REGEX_DETAIL);
            Calendar startCalendar = DateUtil.stringToCalendar(BoxConfigOfflineV1.DATE_START, "yyyyMMdd");
            Calendar endCalendar = DateUtil.stringToCalendar(BoxConfigOfflineV1.DATE_END, "yyyyMMdd");
            Calendar indexCalendar = null;
            for (indexCalendar = (Calendar) startCalendar.clone(); indexCalendar.before(endCalendar); indexCalendar.set(Calendar.DATE, indexCalendar.get(Calendar.DATE) + 1)) {
...
//                                boxInfoBean.getMac(),
    ...
    private String getOssKeyPrintString(String ossObjectKey) {
        ...
        Pattern pattern = Pattern.compile("^log/box-standalone/\\d+/(\\d+)/([a-zA-Z0-9]{12})_(\\d+)\\.blog\\.zip$");
